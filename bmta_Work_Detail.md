...
실시간 갱신: 버스/지하철 실시간 데이터는 30초 주기로 폴링(Polling)하며 클라이언트 캐싱을 적용합니다.


### 6. Firebase & 데이터 무결성 (Integrity)

포인트 차감 트랜잭션: 쪽지 발송 로직 구현 시, 반드시 Firebase Firestore Transaction을 사용한다. (네트워크 오류 등으로 포인트만 깎이고 쪽지가 안 가는 상황 방지)

Boom Up 취소 로직: likes 카운트와 작성자의 points는 동기화되어야 한다. Cloud Functions를 사용하거나, 클라이언트에서 Batch 작성 시 이 두 데이터가 동시에 변경되도록 작성한다.
...
### 8. 공공 데이터 API 표준화 (API Normalization)

지역마다(서울, 경기, 부산 등) API 데이터 형식이 다를 수 있다.

AI에게 "모든 API 응답은 lib/models/에 정의된 공통 TransitData 모델로 변환(Parsing)하여 사용하도록" 지시한다.
